...
<div class="mb-4 d-flex gap-2 flex-wrap">
  <a href="{{ url_for('aggiungi_prodotto') }}" class="btn btn-success">➕ Aggiungi Prodotto</a>
  <a href="{{ url_for('gestisci_categorie') }}" class="btn btn-secondary">🗂️ Gestisci Categorie</a>
</div>

<div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4 mb-5" id="categorieCardContainer">
  {% set immagini_categorie = {
      "Alimentari": "alimentari.jpg",
      "Carne Fresca": "carne_fresca.jpg",
      "Carne Gelo": "carne_gelo.jpg",
      "Ittico Gelo": "ittico_gelo.jpg",
      "No Food": "no_food.jpg",
      "Ittico Fresco": "ittico_fresco.jpg"
  } %}
  {% if categorie %}
    {% for categoria in categorie %}
      <div class="col">
        <div class="card shadow-sm hover-scale cursor-pointer border-0 text-center position-relative categoria-card"
             style="background-image: url('{{ url_for('static', filename='uploads/categorie/' ~ immagini_categorie.get(categoria, 'default_categoria.jpg')) }}');"
...
<a href="{{ url_for('index') }}" class="btn btn-secondary mt-5">🏠 Torna alla home</a>
{% endblock %}
...
#listaProdotti .list-group-item i {
  font-size: 1.2rem;
}

#listaProdotti .list-group-item:hover {
