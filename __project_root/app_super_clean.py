...
def index():
    with get_db() as conn:
        ...
        now = datetime.now()
        oggi_data = now.date()
        ultimo_mese_completo = now.replace(day=1) - relativedelta(days=1)
        mese_corr = ultimo_mese_completo.month
        anno_corr = ultimo_mese_completo.year
        mese_prec_dt = ultimo_mese_completo - relativedelta(months=1)
        mese_prec = mese_prec_dt.month
        anno_prec = mese_prec_dt.year
        cur.execute("SELECT COALESCE(SUM(totale),0) AS totale FROM fatturato WHERE mese=%s AND anno=%s", (mese_corr, anno_corr))
        fatturato_corrente = float(cur.fetchone()['totale'])
        cur.execute("SELECT COALESCE(SUM(totale),0) AS totale FROM fatturato WHERE mese=%s AND anno=%s", (mese_prec, anno_prec))
        fatturato_precedente = float(cur.fetchone()['totale'])
        variazione_fatturato = 0
        if fatturato_precedente > 0:
            variazione_fatturato = round(((fatturato_corrente - fatturato_precedente) / fatturato_precedente) * 100, 1)
        trenta_giorni_fa = now - timedelta(days=30)
        cur.execute("SELECT id, nome, zona, data_registrazione FROM clienti WHERE data_registrazione >= %s", (trenta_giorni_fa,))
        clienti_nuovi_rows = cur.fetchall()
        clienti_nuovi = len(clienti_nuovi_rows)
        clienti_nuovi_dettaglio = [dict(r) for r in clienti_nuovi_rows]
        ...
        for cliente in clienti_rows:
            ...
            if stato_db and stato_db != 'automatico':
                ...
                if giorni <= 60: stato = "attivo"
                elif 61 <= giorni <= 91: stato = "bloccato"
                else: stato = "inattivo"
            info = {"id": cliente["id"], "nome": cliente["nome"], "ultimo_fatturato": ultimo, "giorni": giorni}
            if stato == "attivo": clienti_attivi_dettaglio.append(info)
            elif stato == "bloccato": clienti_bloccati_dettaglio.append(info)
            else: clienti_inattivi_dettaglio.append(info)
        clienti_bloccati = len(clienti_bloccati_dettaglio)
        clienti_bloccati_inattivi = len(clienti_bloccati_dettaglio) + len(clienti_inattivi_dettaglio)
        clienti_bloccati_inattivi_dettaglio = (
            [{"id": c["id"], "nome": c["nome"], "stato": "bloccato"} for c in clienti_bloccati_dettaglio] +
            [{"id": c["id"], "nome": c["nome"], "stato": "inattivo"} for c in clienti_inattivi_dettaglio]
        )
        primo_giorno_mese = now.replace(day=1).date()
        cur.execute('''
            SELECT c.nome AS cliente, p.nome AS prodotto, cp.data_operazione
            FROM clienti_prodotti cp JOIN clienti c ON cp.cliente_id = c.id JOIN prodotti p ON cp.prodotto_id = p.id
            WHERE cp.lavorato = TRUE AND cp.data_operazione >= %s
        ''', (primo_giorno_mese,))
        prodotti_inseriti = [dict(r) for r in cur.fetchall()]
        prodotti_totali_mese = len(prodotti_inseriti)
        cur.execute('''
            SELECT c.nome AS cliente, p.nome AS prodotto, pr.data_rimozione
            FROM prodotti_rimossi pr JOIN prodotti p ON pr.prodotto_id = p.id JOIN clienti c ON pr.cliente_id = c.id
            WHERE pr.data_rimozione >= %s
        ''', (primo_giorno_mese,))
        prodotti_rimossi = [dict(r) for r in cur.fetchall()]
        prodotti_rimossi_mese = len(prodotti_rimossi)
        cur.execute("SELECT anno, mese, COALESCE(SUM(totale),0) as totale FROM fatturato GROUP BY anno, mese ORDER BY anno DESC, mese DESC LIMIT 12")
        fatturato_mensile = {f"{r['anno']}-{r['mese']:02}": float(r['totale']) for r in reversed(cur.fetchall())}
        cur.execute("SELECT COALESCE(c.zona,'Sconosciuta') AS zona, COALESCE(SUM(f.totale),0) AS totale FROM fatturato f JOIN clienti c ON f.cliente_id=c.id GROUP BY c.zona ORDER BY zona")
        fatturato_per_zona = {r["zona"]: float(r["totale"]) for r in cur.fetchall()}
        cur.execute("SELECT v.id, c.nome as cliente_nome, v.ora_visita, v.completata FROM visite v JOIN clienti c ON v.cliente_id=c.id WHERE v.data_visita=%s ORDER BY v.ora_visita DESC", (oggi_data,))
        visite_oggi = cur.fetchall()
        notifiche = []
        dismissed = session.get('dismissed_notifiche', [])
        notifiche = [n for n in notifiche if n.get('id') not in dismissed]
    return render_template(
        "02_index.html",
        variazione_fatturato=variazione_fatturato,
        clienti_nuovi=clienti_nuovi, clienti_nuovi_dettaglio=clienti_nuovi_dettaglio,
        clienti_bloccati=clienti_bloccati, clienti_bloccati_dettaglio=clienti_bloccati_dettaglio,
        ...
        prodotti_totali_mese=prodotti_totali_mese, prodotti_rimossi_mese=prodotti_rimossi_mese,
        prodotti_inseriti=prodotti_inseriti, prodotti_rimossi=prodotti_rimossi,
        fatturato_mensile=fatturato_mensile, fatturato_per_zona=fatturato_per_zona,
        notifiche=notifiche, visite_oggi=visite_oggi,
...
def clienti():
    ...
    with get_db() as db:
        ...
        for c in clienti_list:
            ...
            if c.get('_fatt_ref_val'):
                kpi['fatturato_mese_corrente'] += float(c['_fatt_ref_val'])
            if c.get('_fatt_ref_prec_val'):
...
def nuovo_cliente():
        ...
        with get_db() as db:
            ...
            giorno_visita = request.form.get('giorno_visita_standard')
...
def modifica_cliente(id):
    ...
    with get_db() as db:
        ...
        # Prodotti
        cur.execute('''
            SELECT p.id, p.codice, p.nome, p.categoria_id, c.nome AS categoria_nome
        ...
        if request.method == 'POST':
            ...
            if mese and anno and importo:
                ...
                if esist:
                    ...
                    d_ini = current_datetime if (lavorato_now and not v_lav) else None
                    ...
                    d_ini = current_datetime if lavorato_now else None
        ...
        fatturato_mensile = {f"{r['anno']}-{r['mese']:02d}": r['totale'] for r in cur.fetchall()}
            ...
            for p in prodotti:
                ...
                if pid in assoc_dict:
                    ...
                    prodotti_data_inizio[str(pid)] = assoc_dict[pid]['data_inizio_lavorazione']
                    ...
                    prodotti_data_inizio[str(pid)] = None
